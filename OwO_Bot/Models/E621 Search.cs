...
namespace OwO_Bot.Models
{
    ...
    public class Tags
    {
        ...
        [JsonIgnore] public List<string> MasterList {
            get
            {
                if (_masterList.Count == 0)
                {
                    _masterList.AddRange(General);
                    _masterList.AddRange(Species);
                    _masterList.AddRange(Character);
                    _masterList.AddRange(Copyright);
                    _masterList.AddRange(Character);
                    _masterList.AddRange(Artist);
                    _masterList.AddRange(Invalid);
                    _masterList.AddRange(Lore);
                    _masterList.AddRange(Meta);
                }
        ...
        private List<string> _masterList { get; set; } = new List<string>();
